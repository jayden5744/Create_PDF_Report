...
def convert_sa09(sa09_name, sa09_title, sa09_description, sa09_path, sa09_save_path):
    ...
    for sheet_name in csv_name:
        ...
        value = []
        x_axis, y_axis, graph_title = pd.DataFrame()
...
def convert_sa14(sa14_name, sa14_title, sa14_description, sa14_path, sa14_save_path):
    print '-----------------SA9 연구파일 pdf변환을 시작합니다.---------------------'.decode('utf-8')
    # data_only = True로 해줘야 수식이 아닌 값으로 받아온다.
    if sa14_path is None:  # 파일과 같은 폴더에 있을 때
        try:
            load_wb = load_workbook(sa14_name + ".xlsx")
        except IOError:  # xls파일인 경우
            xls2xlsx(sa14_name, sa14_path)
            load_wb = load_workbook(sa14_name + ".xlsx")
    else:  # 따로 경로를 지정했을 때
        try:
            load_wb = load_workbook(str(sa14_path) + '/' + sa14_name + ".xlsx")
            print(str(sa14_path) + '/' + sa14_name + ".xlsx")
        except IOError:  # xls파일인 경우
            xls2xlsx(sa14_name, sa14_path)
            load_wb = load_workbook(str(sa14_path) + '/' + sa14_name + ".xlsx")
    print '-----------------Excel File을 성공적으로 불러왔습니다.---------------------'.decode('utf-8')
    ...
    # docx파일을 생성을 위한 save('파일명')
    document.save(sa14_name.decode('utf-8') + '.docx')
    print '-----------------Docs File을 성공적으로 불러왔습니다.---------------------'.decode('utf-8')

    # 파일 경로 절대경로로
    in_file = os.path.abspath(sa14_name.decode('utf-8') + '.docx')
    if sa14_save_path is None:
        out_file = os.path.abspath(sa14_name)
    else:
        out_file = os.path.abspath(str(sa14_save_path) + '\\' + sa14_name)
    # word형식의 파일을 열기
    word = comtypes.client.CreateObject('Word.Application')
    doc = word.Documents.Open(in_file)
    # PDF형식으로 저장
...
if __name__ == '__main__':
    ...
    # -------------------------------------------------------------------------------------------------------------- #
    while args.sa09 or args.sa10 or args.sa12 or args.sa13 or args.sa14:
